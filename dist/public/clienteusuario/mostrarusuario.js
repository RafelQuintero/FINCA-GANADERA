...
document.getElementById("get-usuario").addEventListener("submit", async (e) => {
  ...
  const form = document.getElementById("get-usuario");
  ...
  try {
    const response = await fetch(
      `http://localhost:8000/api/usuarios/${data.id}`,
      {
        method: "GET",
      }
    );
    if (response.ok) {
      ...
      const usuarioFinca = await response.json();
      ...
      const usuariolist =
        !Array.isArray(usuarioFinca) && usuarioFinca.usuario
          ? usuarioFinca.usuario
          : [];
      ...
      if (usuariolist.length === 0) {
        alert("No hay usuario disponible con ese id para mostrar.");
        return;
      }
      ...
      usuarioDiv.style.marginBottom = "10px"; // Espacio entre los elementos

      usuarioDiv.innerHTML = `
          <strong>ID:</strong> ${usuariolist.id} <br>
          <strong>Nombres:</strong> ${usuariolist.nombre} <br/>
          <strong>Apellidos:</strong> ${usuariolist.apellido} <br/>
          
          <strong>"id" de la finca:</strong> ${usuariolist.fincaId} <br/>

        `;
      resultsContainer.appendChild(usuarioDiv);
    ...
    alert("Error de conexion. Verifica si el servidor esta corriendo ", error);
  }
});
